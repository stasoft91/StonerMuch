    ...
    <div v-for="(dayPuffs, date) in puffsByDay" :key="date" class="day-logs">
      ...
      <div>
        ...
        <div class="total">
          <p
              v-if="isTHCVisible"
              class="thc">THC 🤙 {{ (weightOfPuffs(dayPuffs) * percentTHC/100).toFixed(2) }} g</p>
          <p class="gr">{{ date }} 🤙 {{ weightOfPuffs(dayPuffs).toFixed(2) }} g</p>
        </div>
      </div>
    </div>
...
const weightOfPuffs = (dayPuffs: Puff[]) => {
